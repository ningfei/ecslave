...
typedef struct PACKED
{
	/** length of EtherCAT datagram 12 bits */
	uint16	elength; /* The Ethercat header */
	...
	/** ADO Address Offset */
	uint16  ADO;
	/** length of data portion in datagram (12bit) */
	uint16  dlength;
...
enum 
{
    ...
    ECT_BASE_REVISION,	/* base revision. 1byte */
    ECT_BASE_BUILD1,	/* base build .2bytes	*/
    ECT_BASE_BUILD2,   /* base fmmu count 1byte		*/
    ECT_BASE_FMMUS,    /* base sync count 1byte */
    ECT_BASE_SYNCM,								/* octet 1	0x000F one port. mii */
