...
int ecs_get_local_conf(e_slave *slave)
{
	...
	if (ret < 0) {
		...
		printf("LOCAL SUBNET MASK %s\n",
					inet_ntop(AF_INET, &sin_mask.sin_addr, temp_str, sizeof(temp_str)));
	}
	...
	sprintf(slave->macaddr,
			...
			slave->mac.ether_shost[4],
			slave->mac.ether_shost[5]);
	printf("LOCAL MAC %s\n",slave->macaddr);
	return 0;
}
...
void ecs_rx_packet(e_slave* slave)
{
	...
	while(1) {
		...
		slave->pkt_size = recvfrom(slave->m_recvsock,
				...
				(struct sockaddr *)&slave->m_addr, &addr_size);
	...
	printf("rx packet: bytes read=%d element"
			"index=0x%x size=%d data length=%d\n",
			slave->pkt_size,
			slave->pkt_index,
...
{
//	uint8_t *ecthdr = (uint8_t *)&slave->pkt[sizeof(struct ether_header)];
